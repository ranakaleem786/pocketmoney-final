...
export default function PWAInstallBanner({
  ...
  const handleInstall = async () => {
    // agar browser support nahi karta to silently ignore
    if (!deferredPrompt) {
    ...
    <div className="flex justify-center mx-2 mt-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-green-600 text-white px-4 py-3 rounded-2xl shadow-xl flex items-center justify-between gap-3">
          ...
          {/* INSTALL BUTTON (ALWAYS ACTIVE) */}
          <button
            onClick={handleInstall}
            className="bg-white cursor-pointer text-green-700 px-3 py-1 rounded-lg font-semibold text-sm"
          >
            {buttonText}
          </button>
